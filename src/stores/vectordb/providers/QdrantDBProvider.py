...
class QdrantDBProvider(VectorDBInterface):
    ...
    def connect(self):
        try:
            ...
            # Modified to properly connect to either local path or host:port
            if self.db_path.startswith("localhost") or ":" in self.db_path:
                host, *port_parts = self.db_path.split(":")
                port = int(port_parts[0]) if port_parts else 6333
                self.client = QdrantClient(host=host, port=port)
                self.logger.info(f"Connected to Qdrant at host: {host}, port: {port}")
            else:
                self.client = QdrantClient(path=self.db_path)
                self.logger.info(f"Connected to Qdrant at local path: {self.db_path}")
